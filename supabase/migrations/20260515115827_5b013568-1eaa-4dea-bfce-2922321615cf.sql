...
BEGIN
  ...
  IF sess.status = 'billed' THEN
    SELECT * INTO existing_order
     ...
     WHERE o.session_id = sess.id AND o.pos_account_id = p_account_id
    ...
    ELSE
      DELETE FROM public.pos_order_payments p WHERE p.order_id = existing_order.id;
      DELETE FROM public.pos_order_items oi WHERE oi.order_id = existing_order.id;
      DELETE FROM public.pos_orders o WHERE o.id = existing_order.id;
  ...
  IF NOT is_pending THEN
    ...
    VALUES (v_order_id, p_account_id, p_payment_method, sess.total);
  ...
  RETURN json_build_object('success', true, 'order_id', v_order_id, 'order_number', order_num, 'payment_status', effective_status);
END;
$function$;